"""
Remove image columns from nu_full.
"""

from yoyo import step

OLD_NU_FULL = """CREATE OR REPLACE VIEW `nu_full` AS
  select
...
NEW_NU_FULL = """CREATE OR REPLACE VIEW `nu_full` AS
  select
    `NU`.`nu_graphic` AS `nu_graphic`,
    `NU`.`first_language` AS `first_language`,
    `NU`.`survey_language` AS `survey_language`,
